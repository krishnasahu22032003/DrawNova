...
export default function RoomHeader({ roomId, userId, creatorId }: RoomHeaderProps) {
                ...
                {open && (
                  ...
                  >
                    ...
                    {isCreator && (
                      <button
                        disabled={loading}
                        onClick={() => { setOpen(false); handleDeleteRoom(); }}
                        className="flex w-full items-center cursor-pointer gap-3 rounded-xl px-4 py-3 text-sm font-medium text-red-600 transition-all duration-300 hover:bg-red-500/10"
                      >
                        <X className="h-4 w-4" />
                        {loading ? "Please Wait..." : "Delete Room"}
                      </button>
                    )}
                  </motion.div>
                )}
          ...
          <>
            ...
            >
              ...
              <div className="flex flex-col gap-3">
                ...
                {isCreator && (
                  <button
                    onClick={() => { setMobileMenuOpen(false); handleDeleteRoom(); }}
                    disabled={loading}
                    className="flex items-center gap-3 rounded-2xl border cursor-pointer border-red-700/20 bg-red-700/10 px-5 py-4 text-sm font-medium text-red-600 transition-all duration-300 hover:bg-red-700/15"
                  >
                    <X className="h-5 w-5" />
                    {loading ? "Please Wait..." : "Delete Room"}
                  </button>
